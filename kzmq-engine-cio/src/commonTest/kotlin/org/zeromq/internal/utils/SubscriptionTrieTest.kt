...
val SubscriptionTrieTest by testSuite {
    test("doesn't match null subscription") {
        subscriptionTest("any", setOf<Int>()) { it }
    }

    test("matches empty subscription") {
        subscriptionTest("any", setOf(1)) {
            it.add("", 1)
        }
    }

    test("matches only prefixes") {
        subscriptionTest("a", setOf(1)) {
            it.add("a", 1).add("ab", 2)
        }
    }

    test("matches all prefixes") {
        subscriptionTest("abc", setOf(1, 2)) {
            it.add("a", 1).add("ab", 2)
        }
    }

    test("matches multiple same subscriptions only once") {
        subscriptionTest("abc", setOf(1)) {
            it.add("a", 1).add("a", 1)
        }
    }

    test("matches multiple subscriptions only once") {
        subscriptionTest("abc", setOf(1)) {
            it.add("a", 1).add("ab", 1)
        }
    }

    test("multiple subscriptions are taken in account") {
        subscriptionTest("abc", setOf(1)) {
